...
OldGame::OldGame(HINSTANCE instanceHandle, HWND windowHandle) {
...
ResourceContext OldGame::LoadResources() const {
...
RECT OldGame::start(const Difficulty difficulty) {
    switch (difficulty) {
...
RECT OldGame::start(const int32_t width, const int32_t height, const int32_t mines) {
    ...
    for (int32_t row = 0; row < this->rowCount; row++) {
        std::vector<std::unique_ptr<OldCell>> cellRow;

        for (int32_t column = 0; column < this->columnCount; column++) {
            cellRow.push_back(std::make_unique<OldCell>(
                *this,
    ...
    for (int32_t row = 0; row < this->rowCount; row++) {
        for (int32_t column = 0; column < this->columnCount; column++) {
            ...
            for (const auto [deltaColumn, deltaRow]: EIGHT_DIR_CELL_OFFSETS) {
                ...
                if (const OldCell *cell = this->cells[newRow][newColumn].get(); cell != nullptr && cell->hasMine()) {
...
void OldGame::revealConnectedEmptyCells(int32_t selectedCellX, int32_t selectedCellY) const {
    ...
    while (!queue.empty()) {
        ...
        const std::unique_ptr<OldCell> &cell = this->cells[row][column];
...
void OldGame::showMines() const {
    for (int32_t row = 0; row < this->rowCount; row++) {
        for (int32_t column = 0; column < this->columnCount; column++) {
            if (OldCell *cell = this->cells[row][column].get(); cell != nullptr && cell->hasMine()) {
                cell->revealCell();
            }
        }
    }
...
void OldGame::showCounts() const {
    for (int32_t row = 0; row < this->rowCount; row++) {
        for (int32_t column = 0; column < this->columnCount; column++) {
            if (OldCell *cell = this->cells[row][column].get(); cell != nullptr && cell->getSurroundingMineCount() > 0 && !cell->hasMine()) {
                cell->revealCell();
            }
        }
    }
...
void OldGame::revealAll() const {
    for (int32_t row = 0; row < this->rowCount; row++) {
        for (int32_t column = 0; column < this->columnCount; column++) {
            if (OldCell *cell = this->cells[row][column].get(); cell != nullptr) {
                cell->revealCell();
            }
        }
    }
