...
LRESULT OldCell::BoxProc(
    ...
    const auto box = reinterpret_cast<OldCell *>(cellPointer);
...
void OldCell::revealCell() {
    logger->debug("Cell::reveal");
    this->state = State::REVEALED;
    InvalidateRect(this->getHandle(), nullptr, true);
}

void OldCell::setSurroundingMineCount(const int32_t count) {
    this->surroundingMineCount = count;
}

void OldCell::DrawBorder(HDC hdc, LPRECT rect) {
